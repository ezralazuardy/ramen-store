...
const storeData = (ramen_categories, ramen_stores) => {
    form.category = category.value ? category.value.id : ramen_categories[0].id;
    form.store = store.value ? store.value.id : ramen_stores[0].id;
    form.post(route("ramens.store"), {
        errorBag: "storeData",
        preserveScroll: true,
        onError: () => {
    ...
    <AppLayout title="Ramens">
        ...
        <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
            <FormSection @submitted="storeData(ramen_categories, ramen_stores)">
                <template #title>Add Ramen</template>

                <template #description>
                    Add a new ramen data to database.
                </template>
